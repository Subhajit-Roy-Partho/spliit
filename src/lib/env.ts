...
const envSchema = z
  .object({
    ...
    // Point at https://nano-gpt.com/api/v1 to use NanoGPT.
    ...
    // Legacy NanoGPT variables (Dhar fork) — honoured as a fallback for
    // OPENAI_API_KEY / OPENAI_BASE_URL so existing deployments keep working.
    NANOGPT_API_KEY: z.string().trim().optional(),
    NANOGPT_BASE_URL: z.preprocess(
      interpretBlankEnvVarAsUndefined,
      z.string().trim().url().optional(),
    ),
    NANOGPT_MODEL_FAST: z.string().trim().optional(),
    NANOGPT_MODEL_ACCURATE: z.string().trim().optional(),
  ...
  .superRefine((env, ctx) => {
    ...
    if (
      ...
      ctx.addIssue({
        code: ZodIssueCode.custom,
        message:
          'If ENABLE_RECEIPT_EXTRACT or ENABLE_CATEGORY_EXTRACT is set, then OPENAI_API_KEY (or legacy NANOGPT_API_KEY) must be set too',
      })
    }
