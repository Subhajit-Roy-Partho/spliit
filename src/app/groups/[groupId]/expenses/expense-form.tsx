...
export function ExpenseForm({
  ...
  useEffect(() => {
    ...
    // `sign` has a zod default, so the raw form value types it as optional —
    // normalize before passing items to helpers that need full ExpenseItems.
    const items: ExpenseItem[] = (form.getValues('items') ?? []).map((i) => ({
      ...i,
      sign: i.sign ?? ('+' as const),
    }))
